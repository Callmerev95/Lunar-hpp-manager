...
export function MaterialForm() {
  const [error, formAction, pending] = useActionState<
    ...
    FormData
  >(createMaterialAction, undefined);
  const [kind, setKind] = useState<string>("raw");
  const [buyUnit, setBuyUnit] = useState<string>("gram");
  ...
  return (
    <form action={formAction} className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="font-heading text-xl">Bahan baru</CardTitle>
          <CardDescription>
            Catat bahan yang kamu beli. Harga boleh diisi nanti, tapi resep
            baru bisa dihitung kalau bahan sudah punya harga.
          </CardDescription>
        </CardHeader>
